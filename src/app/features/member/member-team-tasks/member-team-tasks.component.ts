...
export class MemberTeamTasksComponent implements OnInit {
  ...
  loadTeamTasks(teamId: number, pageNumber: number = 1, pageSize: number = 10): void {
    this.loading = true;
    this.taskService.getTeamTasks(teamId, pageNumber, pageSize).subscribe({
      next: (res) => {
  ...
  onStatusUpdated(dto: UpdateTaskStatusDto): void {
    ...
    this.taskService.updateStatus(this.selectedTask.id, dto).subscribe({
      next: (res) => {
        if (res.isSuccess) {
          this.swal.success('Status updated successfully', 'Team Management');
          const teamId = this.authState.currentUser()?.teamId;
          if (teamId) this.loadTeamTasks(teamId);
        } else {
          this.swal.error(res.message || 'Failed to update status', 'Team Management');
        }
      },
      error: (err) => {
        this.swal.error('An unexpected error occurred', 'Error');
